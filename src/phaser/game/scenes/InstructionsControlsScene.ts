...
export class InstructionsControlsScene extends BaseScene {
    ...
    init() {
        ...
        const headerText = this.add.text(0, 0, "Instructions/Controls", headerTextFontConfig)
            .setOrigin(0.5);
        
        const bodyLine1 = this.add.text(0, 80, "- Press left arrow key to move player ship left\n- Press right arrow key to move player ship right\n- Press spacebar to fire weapon upwards when weapon power bar is green\n- Press 'n' key to fire Nova Blast when special power bar is green\n- You will lose a life every time a ufo reaches the bottom of the screen\n- Your score will increase by one every time you destory a ufo", bodyTextFontConfig)
            .setOrigin(0.5);
        
        const backButton = this.add.text(0, 150, "Back", headerTextFontConfig)
            .setOrigin(0.5)
            .setInteractive();
        ...
        this.textContainer = this.add.container(this.gameWidth / 2, this.gameHeight / 3, [
            headerText,
            bodyLine1,
            backButton
        ]);
